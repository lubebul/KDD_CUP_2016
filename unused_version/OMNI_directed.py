...
class PrepareGraph:
    ...
    def plotGraph(self, graph, outfile, title):
        G = snap.TNGraph.New()
        for i in range(self.N):
            G.AddNode(i)
        # add name to nodes
        ...
        snap.SaveGViz(G, outfile, title, True, S)
...
for x in range(3):
    ...
    # omni.pre.plotGraph(omni.graph, '{0}.dot'.format(Confs[x]), '{0}'.format(Confs[x])) # plot graph -> dot file
